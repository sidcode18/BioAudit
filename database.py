...
class Contraindication(db.Model):
    ...
    engine_action = db.Column(db.String(20), nullable=False) 
...
def seed_data():
    ...
    biomarkers = [
        ...
        Biomarker(name="LDL Cholesterol", clinical_min=0.0, clinical_max=100.0, athletic_min=0.0, athletic_max=80.0, unit="mg/dL"),
        Biomarker(name="TSH", clinical_min=0.46, clinical_max=4.68, athletic_min=1.0, athletic_max=2.5, unit="mIU/L"),
    ]
    ...
    contraindications = [
        ...
        Contraindication(marker_name="LDL Cholesterol", supplement_name="Omega-3 Fish Oil", condition_type="HIGH", engine_action="INCREASE", warning_text="Elevated LDL impairs endothelial function, restricting blood flow and cardiovascular endurance. ", maintenance_dose=2.0, deficit_multiplier=0.05, unit_label="g", loading_protocol="Ensure high EPA/DHA ratio."),
        Contraindication(marker_name="TSH", supplement_name="L-Glutamine", condition_type="HIGH", engine_action="REDUCE", warning_text="Sub-optimal thyroid function drastically reduces basal metabolic rate, hindering the year-long protocol to reach the 80kg weight class and reducing explosive power.", maintenance_dose=0.0, deficit_multiplier=0.0, unit_label="g", loading_protocol=None),
    ]
