...
def audit_user_data(extracted_data: dict, user_supplements: dict, custom_rules: list = None) -> dict:
    ...
    for marker_name, value in extracted_data.items():
        ...
        if direction:
            contraindications = list(Contraindication.query.filter_by(marker_name=marker_name, condition_type=direction).all())
            
            for cr in custom_rules:
                if cr.marker_name == marker_name and cr.condition_type == direction:
                    contraindications.append(cr)
